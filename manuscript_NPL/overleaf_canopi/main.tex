...
% Self-contained: \input{related_work}; all other sections inline.
...
\emph{[PROVISIONAL---runs configured, pending.]} To attribute the recovery to the
invariance machinery rather than the partial-AUC term we compare, on the monolingual
encoder, invariance-only (B6) against pAUC-only (B7). The pre-registered prediction
is that B6 reproduces the lift while B7 stays collapsed; the configurations
(\texttt{b6\_mono}, \texttt{b7\_mono}) are released and the table will be filled from
the run.
